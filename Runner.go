	...
	for {
		for !claiming {
			...
			if err := Client.Do(req, res); err == nil {
				...
				} else if strings.Contains(BodyC, `Please wait a few minutes before you try again.`) || fasthttp.AcquireResponse().StatusCode() != 200 {
	...
	for {
		for !claiming {
			...
			if err := Client.Do(req, res); err == nil {
				...
				// fmt.Println(BodyC)
				atomic.AddInt32(&Counter.Attempt, 1)
				if fasthttp.AcquireResponse().StatusCode() == 200 && !strings.Contains(BodyC, `Please wait a few minutes before you try again.`) {
